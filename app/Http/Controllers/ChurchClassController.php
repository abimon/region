...
class ChurchClassController extends Controller
{
    ...
    public function class_data($id)
    {
        ...
        if (request()->is('api/*')) {
            return response()->json(['message' => 'Class retrieved successfully', 'class' => $class, 'members' => $members->sortByAsc('details.name'), 'lessons' => $lessons, 'announcements' => $announcements, 'assignments' => $assignments], 200);
        }
