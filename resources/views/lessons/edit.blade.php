@extends('layouts.app', ['title' => 'Edit Lesson'])
@section('content')
<div class="container">
    <form action="{{route('lessons.update', $lesson->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col-md-4">
                    ...
                    <div class="ms-2 card p-3 border-dark bg-transparent shadow h-75" style="border-style:dashed;">
                        <img id="outCard" src="{{ $lesson->image }}" style="height: 100%; object-fit:contain;" />
                ...
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <small id="titleHelp" class="form-text text-muted">Enter the title of the lesson.</small>
                        @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="category" id="" class="form-select text-dark bg-white" required>
                            ...
                            <option value="{{$option}}" {{ $lesson->category == $option ? 'selected' : '' }}>{{$option}}</option>
                            @endforeach
                        </select>
                        <small id="titleHelp" class="form-text text-muted">Select the category of the lesson.</small>
                        @error('title')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="post" name="content" rows="3">{{ $lesson->content }}</textarea>
                @error('content')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
